    ...
    if(data.length==2){
        ...
        data.forEach((input)=>{
            if(!input.value){
                ...
                input.nextElementSibling.innerText='Veuillez entrer un nom d\'utilisateur valide d\'au moins 5 caractères';
                keyUp(input);
            } else if(input.name=='mdp' && !(/^[a-zA-Z0-9]{8,50}$/.test(input.value))){
                isOk=false;
                input.classList.add('error');
                input.nextElementSibling.innerText='Veuillez entrer un mot de passe valide d\'au moins 8 caractères';
                keyUp(input);
...
if (data.length==4) {
    form.mdp.addEventListener('keyup',(e)=>{
        if(form.mdp.value && form.mdp_confirm.value != form.mdp.value){
            form.mdp_confirm.nextElementSibling.innerText='Les mots de passe ne correspondent pas';
            form.mdp_confirm.nextElementSibling.style.color='#dc3545';
            isOk=false;
        } else{
            form.mdp_confirm.nextElementSibling.innerText='Les mots de passe correspondent';
            ...
            if (xhr.status==200 && xhr.readyState==4) {
                if(xhr.responseText=='wrong data'){
                    swal({
                        ...
                        text: "Les identifiants entrés sont incorrects. Veuillez entrez à nouveau vos identifiants!",
                        icon: "error",
                        button: "OK",
                    });
                    ...
                    swal({
                        ...
                        text: "Vos identifiants ont étés enregistrés! Cliquez sur OK pour être redirigés",
                        icon: "success",
                        button: "OK",
                    }).then(()=>{
                        window.location.href= window.location.href.replace(/inscript\.php/,xhr.responseText);
                    });
